...
const poll = {
  ...
  registerNewAnswer() {
    ...
    if (inputNum >= 0 && inputNum < this.options.length) {
      this.answers[inputNum]++;
    }
    //4. Run the 'displayResults' method
    this.displayResults();
    this.displayResults('string');
  },

  //3. Create a method 'displayResults'
  displayResults(typeOfOutput = 'array') {
    // const typeOfOutput = prompt(`Input Type?\n'string' or 'array'`) ?? 'array';
    // typeOfOutput.toLowerCase().trim();
    if (typeOfOutput === 'array') console.log(this.answers);
    else if (typeOfOutput === 'string')
...
const str1 = 'Alpha';
const str2 = 'Beta';
const str3 = ['0: JavaScript', '1: Python', '2: Rust', '3: C++'];
console.log(str1 + '\n' + str3.join('\n'));
*/
...
const mainFun = function () {
  ...
  document.querySelector('body').addEventListener('click', subFun);
};
